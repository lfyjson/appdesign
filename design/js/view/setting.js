define(function (require, exports, module) {
	...
	Templates.prototype =  {
		...
		createRadio: function () { //单选框设置
			var html = this.tTitleGroup();
				html += this.tTitle('选项', '最多200项，每项最多20个字') + this.tOptionGroup();
				html += this.tTitle('验证', '') + this.tRequire();
			this.setting.innerHTML += html;
		},
		createPageName: function () { //页面设置
			this.setting.innerHTML += this.getPageName();
		},
		getPageName: function () {
			var hideError = this.hideError && (m.strlen(this.data.title) < 10); //隐藏错误样式
		...
		tTitle: function (title, remark, error) {
			return [
				'<div class="setting-title">',
					...
					'<span class="remark '+ (error ? "error" : "") +'">'+ remark +'</span>',
				'</div>'
			].join("");
		},
		...
		tDataType: function () { //日期范围
			var bind = 'data-bind-'+ this.data.id +'="datetype"';
			return this.tItem([
					'<label class="setting-radio"><input type="radio" name="datetype" '+ bind +' value="datetime" '+ (this.data.datetype == "datetime" ? "checked" : "")  +' />年-月-日 时:分</label>',
					'<label class="setting-radio"><input type="radio" name="datetype" '+ bind +' value="date" '+ (this.data.datetype == "date" ? "checked" : "")  +' />年-月-日</label>'
			].join(""));
		},
		...
		tOption: function (value, index) {
			return [
				'<div class="setting-option" data-index="'+ index +'">',
					'<input type="text" class="setting-text" data-model="options" value="'+ value +'" />',
					'<a class="option-delete"  data-model="option-delete"></a>',
					'<a class="option-add"  data-model="option-add"></a>',
				'</div>'
			].join("");
		},
