define(function(require, exports, module) {
    ...
    Setting.prototype = {
        ...
        itemBind: function() { //控件点击事件
            ...
            this.phoneBody.addEventListener('click', function(ev) {
                ...
                if (ev.target.classList.contains('phone-close')) {
                    ...
                    var sId = Delete.deleteItem(ev.target); //删除模板&数据 返回被删除控件id集合
                    Prompt.detection(); //无控件提示
                    _this.onDelCur(sId); //当前控件被删除
                    return;
                }
        ...
        toActive: function(target) {
            ...
            this.id = this.target.dataset.id; //当前控件id
        ...
        showSetting: function(id, hideError) {
            id == 0 ? this.pageNameActive() : this.widgetActive(); //切换设置tab
            //创建设置模板
        ...
        getPhoneItems: function() { /*得到phoneItem数组*/
            this.phoneItems = this.phoneBody.querySelectorAll('div[name="phone-item"]');
        },
        insertPageName: function() { //写入页面设置数据
            this.id = 0;
            Insert.insertPageName();
        },
        onDelCur: function(sId) { //当前控件被删除
