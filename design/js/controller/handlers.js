define(function(require, exports, module) {
    var Model = require('../model/data'); //加载model
    var Templates = require('../view/setting').Templates; //加载控件设置模板
    var Validation = require('./validation'); //验证
    ...
    function init() {
        settingWrap.addEventListener('click', function(ev) {
            ...
            //无行为时跳出
            if (!action) return;
            var id = getCurId(target); //获取当前控件id
    ...
    function optionsAdd(target, data) {
        ...
        var oTemplates = new Templates(data); //获取设置模板
        var oNew = m.parseDOM(oTemplates.tOption('选项' + ++data.optionsIndex))[0]; //获取新选项DOM

        m.insertAfter(oNew, option); //插入新选项模板
        Model.updateOptions(data.id, option.dataset.index, 'add'); //修改选项组模型
        setOptionsIndex(oItem); //重新设置选项组数据索引值
        oItem.classList.remove('limit-delete'); //remove限制删除样式
    ...
    function optionsDelete(target, data) {
        ...
        option.parentNode.removeChild(option); //删除DOM
        Model.updateOptions(data.id, option.dataset.index, 'delete'); //修改选项组模型
        setOptionsIndex(oItem); //重新设置选项组数据索引值
        oItem.classList.remove('limit-add'); //remove限制增加样式
    ...
    function iconHanlder(id, newVal) {
        ...
        // Model.updateData(id, 'curIcon', target.dataset.value);
        // Model.updateData(id, 'iconURL', Search.getIconURL(target.dataset.value));
    }
