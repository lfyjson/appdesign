define(function(require, exports, module) {
    var Phone = require('../view/phone'); /*phone模板*/
    var Model = require('../model/data'); /*model*/
    var Setting = require('./setting').setting; //控件设置模板
    var InsertData = require('../model/insert'); //写入数据方法
    ...
    Drag.prototype = {
        ...
        init: function() {
            ...
            this.parent.onmousedown = function(ev) {
                ...
                document.onmousemove = function(ev) {
                    ...
                    if (_this.isInPhoneInner(ev, _this.phoneInnerLeft, _this.phoneInnerTop, _this.phoneInner)) { /*判断光标是否进入phoneInner*/
                        if (_this.interval) {
                ...
                document.onmouseup = function() {
                    ...
                    if (_this.objName == 'control') {
                        if (_this.placeholder.parentNode) { //在占位符在phoneBody时插入新数据
                            _this.insertNewItem(); /*插入新item*/
                        }
                    } else {
                        _this.setItem(_this.placeholder.parentNode, _this.obj); /*替换item位置*/
                    }
                    Prompt.detection(); //无控件提示
                    _this.itemOrder(); //控件排序
                }
                return false;
            }
        },
        itemOrder: function() { //控件排序
            var aItem = this.phoneInner.querySelectorAll('.phone-body > .phone-item'); //顶级控件
            var aArea = this.phoneBody.querySelectorAll('.phone-area'); //明细

            for (var i = 0; i < aItem.length; i++) {
                Model.updateData(aItem[i].dataset.id, 'order', i + 1); //位置索引
                Model.updateData(aItem[i].dataset.id, 'parentId', 0); //父级id
            }
        ...
        mouseDownInit: function() { //初始化各项参数
            this.type = this.obj.dataset.type;
            this.createShadow();
            this.createPlaceholder();

            this.interval = true; //间隔时间
            ...
            this.getPhoneItems(); /*获取phone项目数组*/
        ...
        setItem: function(oParent, item) {
            ...
            Setting.toActive(item); //切换当前控件设置
        },
        getPhoneItems: function() { /*获取phone项目数组*/
            this.phoneItems = this.phoneBody.querySelectorAll('div[name="phone-item"]');
        },
        findNearest: function(obj, arr, moveEvent) { /*寻找最近的元素*/
            var _this = this;
            var iMin = 1E10;
            var iMinIndex = -1;
        ...
        getDis: function(obj1, obj2, moveEvent) { /*计算直线距离*/
            var a = moveEvent.clientX - m.getLeft(obj2);
            var b = moveEvent.clientY - m.getTop(obj2) + this.phoneInner.scrollTop;
            return Math.sqrt(a * a + b * b);
        },
        isInPhoneInner: function(mouse, left, top, phone) { /*检测光标是否在phoneInner内*/
    ...
    function init() {
        var oControl = document.getElementById('controls'),
            oPhoneBody = document.getElementById('phone-body');
