define(function (require, exports) {
	...
	function tText() {
		...
		return [
			'<div class="form-item-'+ data.type +'" '+ flag +'>',
	...
	function tRadio() {
		...
		return [
			'<div class="form-item-radio" '+ flag +'>',
				'<div class="item-border">',
					'<label for="" class="item-title">',
						data.title,
					'</label>',
					'<span class="item-native-text item-prompt">请选择'+ requireText +'</span>',
					'<select class="item-native" '+ dataId +'>',
						options,
					'</select>',
				'</div>',
			'</div>'
	...
	function tDate() {
		return [
			'<div class="form-item-date"  '+ flag +'>',
				tDateInner(data.title),
			'</div>'
		].join("");
	}
	...
	function tDateRange() {
		return [
			'<div class="form-item-dateRange"  '+ flag +'>',
				tDateInner(data.title),
				tDateInner(data.title2),
			'</div>'
		].join("");
	}
	...
	function tDateInner(title) {
		return [
			'<div class="item-border">',
				...
				'</label>',
				'<span class="item-native-text  item-prompt">请选择'+ requireText +'</span>',
				'<input type="'+ data.datetype +'" class="item-native" '+ dataId +' />',
			'</div>'
		].join("");
	}
	...
	function tImage() {
		...
		return [
			'<div class="form-item-image"  '+ flag +'>',
				'<div class="item-border">',
					'<label for="" class="item-title">',
						data.title,
						requireText,
						'<input type="file" accept="image/*" class="item-native" />',
					'</label>',
					
					'<div class="image-list" '+ dataId +' data-role="photo">',
					'</div>',
				'</div>',
			'</div>'
		].join("");
	...
	function tDetail() {
		return [
			'<div class="form-item-detail"  '+ flag +'>',
				tDetailInner(data.title, 1),
				'<div class="detail-add" data-action="add-detail" data-id='+ data.id +'>'+ data.addDetail +'</div>',
			'</div>'
		].join("");
	}
	...
	function tDetailInner(title, num, isDelete) {
		return [
			...
			'<label for="" class="detail-title">'+ title +'(<em class="detail-num">'+ num +'</em>)</label>',
				isDelete ? '<a class="detail-delete" data-action="delete-detail">删除</a>' : '',
			'</div>',
			'<div class="detail-area"  '+ dataId +'>',
			'</div>'
		].join("");
	}
	...
	function tSubmit() {
		return [
			'<div class="submit-btn-wrap">',
				'<input type="button" value="提交" class="submit-btn" data-action="submit" />',
			'</div>	'
		].join("");
	}
	...
	function tApprover() {
		return [
			'<div class="form-item-approver">',
				'<div class="item-border">',
					'<label class="approver-title">审批人<em>(点击头像可删除)</em></label>',
					'<ul class="approver-list">',
						'<li>',
							'<a class="add-approver" data-action="add-approver"></a>',
						'</li>',
					'</ul>',
				'</div>',
			'</div>'
	...
	function tAddApprover() {
		return [
			'<section class="select-approver">',
				'<div class="approver-header">',
					'<a class="approver-getback"></a>',
					'添加审批人',
				'</div>',
				'<div class="approver-search">',
					'<input type="text" class="approver-search-input" placeholder="搜索姓名">',
				'</div>',
				'<div class="select-approver-list">',
				'</div>',
				'<div class="no-approver"></div>',
			'</section>	'
		].join("");
	}
	...
	// exports.tNoApprover = tNoApprover;
});